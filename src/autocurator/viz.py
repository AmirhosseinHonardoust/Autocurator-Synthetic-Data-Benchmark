...
def corr_heatmaps(df_r: pd.DataFrame, df_s: pd.DataFrame, out_path: str) -> None:
    ...
    if len(num_cols) < 2:
        return
    cr = df_r[num_cols].corr()
    cs = df_s[num_cols].corr()
    fig, axes = plt.subplots(1, 2, figsize=(10, 4))
    sns.heatmap(cr, ax=axes[0], vmin=-1, vmax=1, cmap="coolwarm", cbar=False)
    axes[0].set_title("Real correlation")
    sns.heatmap(cs, ax=axes[1], vmin=-1, vmax=1, cmap="coolwarm", cbar=False)
