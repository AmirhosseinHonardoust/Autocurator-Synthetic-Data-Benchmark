...
from .metrics.privacy import membership_inference_auc, nn_distance_stats
...
def main():
    ...
    if set(df_r.columns) != set(df_s.columns):
        missing = set(df_r.columns) - set(df_s.columns)
        if missing:
            raise ValueError(
                f"Synthetic data is missing columns present in real data: {sorted(missing)}"
            )
